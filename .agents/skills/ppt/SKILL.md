---
name: ppt-expert
description: 演示文稿生成与风格校准专家 (Generate slides with style analysis, visual content checking, data validation, and post-generation style matching)
---
...
* 推荐/关联输入：`.agents/skills/ppt/examples/` 目录下的历史 PPT/PDF 文件（作为前置风格分析的参考）。

## 5. 知识检索规则
生成前需检索与读取：
* `design-system/ppt-guidelines.md` 确定字号与文字层级。
* `design-system/compatibility.md` 确保跨应用兼容。
* `.agents/skills/ppt/examples/` 中的历史 PPT/PDF 文件（至少读取 3 份与当前任务最接近的文件进行前置分析）。

## 6. 执行流程
1. **风格前置分析**：在生成任何 PPT 之前，**必须**读取 `.agents/skills/ppt/examples/` 中至少 3 份最接近的历史 PPT/PDF 文件，并输出包含**页面结构、标题方式、图文比例、常用版式、功能点写法、表格样式和论证顺序**的风格分析。**未完成历史风格分析，绝对不允许生成 PPT**。
...
## 9. 质量检查与 Review 机制
* **核心检查点**：
  - **前置风格分析**：是否已读取 `.agents/skills/ppt/examples/` 中至少 3 份历史 PPT 并输出了风格分析？
